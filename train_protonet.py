...
if __name__ == '__main__':
    ...
    train_sampler = CategoriesSampler(trainset.label, 5, args.way, args.shot + args.query)
    train_loader = DataLoader(dataset=trainset, batch_sampler=train_sampler, num_workers=8, pin_memory=True)

    print("training data loading is done")
    valset = Dataset('val', args)
    val_sampler = CategoriesSampler(valset.label, 10, args.way, args.shot + args.query)
    val_loader = DataLoader(dataset=valset, batch_sampler=val_sampler, num_workers=8, pin_memory=True)
    print("validationb data loading is done")
